...
(function () {
  ...
  function withRequiredFields(items, fields, label) {
    ...
    return items.filter((item) => {
      ...
      if (missing.length) {
        console.warn(
          `[Conecta Vida] Item de "${label}" ignorado — faltando: ${missing.join(", ")}.`,
          item,
        );
        return false;
      }
...
--------------------------------------------------------- */
  function renderContent() {
    document.querySelectorAll("[data-stat-group]").forEach((group) => {
      ...
      if (!raw) {
        console.warn(
          `[Conecta Vida] Nenhum dado encontrado em SITE_DATA para "${key}". Confira data.js.`,
        );
        return;
      }
      ...
      if (key === "plans") {
        const data = withRequiredFields(
          raw,
          ["name", "price", "desc", "features", "cta"],
          "plans",
        );
        group.innerHTML = data
          .map(
    ...
    if (them && us && SITE_DATA.comparison) {
      them.innerHTML = (SITE_DATA.comparison.them || [])
        .map((i) => `<li>${i}</li>`)
        .join("");
      us.innerHTML = (SITE_DATA.comparison.us || [])
        .map((i) => `<li>${i}</li>`)
        .join("");
    }
  }
...
--------------------------------------------------------- */
  function initTestimonials() {
    ...
    const items = withRequiredFields(
    ...
    form.addEventListener("submit", async (e) => {
      ...
      try {
        const log = JSON.parse(
          localStorage.getItem("conectavida_leads") || "[]",
        );
        log.push({
          nome,
          plano,
