...
if (SITE_CONFIG.whatsappNumber === '5511999999999'){
  console.warn('[Conecta Vida] O número de WhatsApp em data.js ainda é o de exemplo. Troque SITE_CONFIG.whatsappNumber antes de publicar.');
}
...
const ICONS = {
  ...
  controller: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6"><rect x="2.5" y="7.5" width="19" height="10" rx="4"/><path d="M7 10v4M5 12h4M15.5 11h.01M18 13h.01"/></svg>',
  people: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6"><circle cx="8.5" cy="8" r="3"/><circle cx="16" cy="9.5" r="2.3"/><path d="M2.5 20c0-3.6 2.7-6 6-6s6 2.4 6 6"/><path d="M14.5 14.3c2.6.4 4.5 2.5 4.5 5.7"/></svg>',
  chart: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6"><path d="M4 20V10M11 20V4M18 20v-7"/><path d="M3 20h18"/></svg>'
};

/**
 * Filtra itens de um array que estejam sem campo obrigatório, em vez de
 * deixar "undefined" aparecer na tela. Avisa no console qual item e qual
 * campo faltou, pra ser fácil de achar o erro de digitação em data.js.
 */
function withRequiredFields(items, fields, label){
  if (!Array.isArray(items)) return [];
  return items.filter(item => {
    const missing = fields.filter(f => item[f] === undefined || item[f] === null || item[f] === '');
    if (missing.length){
      console.warn(`[Conecta Vida] Item de "${label}" ignorado — faltando: ${missing.join(', ')}.`, item);
      return false;
    }
    return true;
  });
...
function renderContent(){
  ...
  document.querySelectorAll('[data-stat-group]').forEach(group => {
    ...
    const raw = SITE_DATA[key];
    if (!raw) {
      console.warn(`[Conecta Vida] Nenhum dado encontrado em SITE_DATA para "${key}". Confira data.js.`);
      return;
    }

    if (key === 'trustBar'){
      const data = withRequiredFields(raw, ['value','label'], 'trustBar');
      group.innerHTML = data.map(item => `
    ...
    if (key === 'problemStats' || key === 'resultStats'){
      const data = withRequiredFields(raw, ['value','label'], key);
      group.innerHTML = data.map(stat => `
        <div class="stat-card">
    ...
    if (key === 'pillars'){
      const data = withRequiredFields(raw, ['title','desc'], 'pillars');
      group.innerHTML = data.map(p => `
        <li class="card-shine">
          <h3>${p.title}</h3>
          <p>${p.desc}</p>
        </li>
    ...
    if (key === 'features'){
      const data = withRequiredFields(raw, ['title','desc'], 'features');
      group.innerHTML = data.map(f => `
    ...
    if (key === 'plans'){
      const data = withRequiredFields(raw, ['name','price','desc','features','cta'], 'plans');
      group.innerHTML = data.map(plan => `
  ...
  if (them && us && SITE_DATA.comparison){
    them.innerHTML = (SITE_DATA.comparison.them || []).map(i => `<li>${i}</li>`).join('');
...
function initTestimonials(){
  ...
  const items = withRequiredFields(SITE_DATA.testimonials, ['quote','name','role'], 'testimonials');
  ...
  // pausa a troca automática quando o usuário passa o mouse ou dá foco no card
  // (conteúdo que muda sozinho precisa de uma forma de pausar — WCAG 2.2.2)
...
function populatePlanSelect(){
  ...
  const options = SITE_DATA.plans
    .map(p => `<option value="${p.name}">${p.name} (${p.price}${p.period})</option>`)
    .join('');
...
function initContactForm(){
  ...
  form.addEventListener('submit', async (e) => {
    ...
    } catch (err){ /* localStorage indisponível — segue o fluxo normalmente */ }
...
function initCursorGlow(){
  ...
  if (!finePointer || reducedMotion) { glow.remove(); return; }

  let x = window.innerWidth / 2, y = window.innerHeight / 2, raf = null;
  function apply(){
...
function initBackToTop(){
  ...
  if (!btn) return;
  function toggle(){ btn.classList.toggle('is-visible', window.scrollY > 700); }
  toggle();
...
})(); // fim do IIFE — nada aqui dentro vaza pro escopo global
